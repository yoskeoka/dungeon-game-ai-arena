...
generated map の default line を legacy small-map baseline から large-map line へ切り替える。

この plan では、以下を成立させる。

- 採用 candidate を large-map line として昇格する
- `seeded-maze-v1` / `rogue-rooms-v1` は legacy small-map baseline として残す
- gameplay default tier、helper default、local bot default、CI/e2e tier をそれぞれ整合させる
...
- `large-map line`
  - この plan で default line へ昇格させる generated ruleset line を指す
- `gameplay default tier`
  - large-map line の実ゲームプレイ default を指す
- `CI/e2e tier`
  - deterministic golden と runtime cost を抑える継続検証 tier を指す
...
- large-map line として採用する ruleset version / selector の導入
- `seeded-maze` / `rogue-rooms` の large-map line candidate 実装と tuned parameter の反映
- helper / local bot / testdata / e2e / docs の default 整合
- medium-size CI/e2e tier と gameplay default tier の共存整理
- legacy small-map baseline の残し方と利用目的の明文化

## Non-Goals

- `0006` で確定した比較条件を再設計すること
- 新規 generator family を追加すること
- legacy small-map baseline ruleset をこの plan で削除すること
- すべての CI lane を gameplay default と同じ大型 size にそろえること

## Code Changes

- `games/dungeon/types.go`
  - promoted large-map line 用 ruleset version、default selector、legacy small-map baseline の位置づけを追加する
...
- `games/dungeon/dungeon_test.go`
  - promoted large-map line と legacy small-map baseline の両方を検証する regression を整備する
- `games/dungeon/botlogic/*.go` と関連 test
  - promoted large-map line 上で既存 bot が探索・記憶・goal 到達を維持できるように調整する
...
- `e2e/dungeon_parity_test.go`
  - canonical deterministic golden を medium-size CI tier で維持しつつ、
    promoted large-map line の regression も確認できるようにする
...
- `docs/specs/dungeon-map-generation.md`
  - default line、legacy small-map baseline、candidate から正式採用へ変わった ruleset を更新する
...
- gameplay default tier が `100x150` 級になっても、
  CI/e2e canonical tier は `40x40` から `50x50` 級を維持して runtime cost を抑える
- legacy small-map baseline は debug / comparative regression のため当面残す

## Sub-tasks

- [ ] promoted large-map line 用 ruleset version / selector と legacy small-map baseline の残し方を spec に固定する
- [ ] [parallel] `seeded-maze` large-map line と `rogue-rooms` large-map line の tuned parameter を実装する
- [ ] [parallel] helper / local bot / testdata の default を promoted line に合わせる
- [ ] [depends on: large-map line 実装, helper / local bot / testdata 更新] deterministic golden、
      medium-size CI/e2e tier、gameplay default tier の test/e2e を更新する
- [ ] [depends on: deterministic golden, CI/e2e tier, gameplay default tier の更新] docs と verification 記録を整合させる
...
- `go test ./e2e/...`
- `go test ./...`
- `go run ./cmd/dungeon-map-helper --ruleset <promoted-large-ruleset> --rng-seed <seed>` で large-map line を確認する
- `go run ./cmd/dungeon-map-helper --ruleset <ci-tier-ruleset> --rng-seed <seed>` で CI/e2e tier の比較を確認する
- promoted large-map line で bot が進行不能にならないこと、
...
- gameplay default tier と CI/e2e tier を分ける判断は、この repo の deterministic verification と
